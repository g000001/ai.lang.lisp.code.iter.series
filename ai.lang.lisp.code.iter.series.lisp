...

(in-package :ai.lang.lisp.code.iter.series.internal)

(provide "AI.LANG.LISP.CODE.ITER.SERIES")
...
(cl:defun install (&key (pkg *package*) (macro T) (shadow T) (implicit-map nil)
  ...
  (when (not remove)
    ...
    (when (not (eq pkg (find-package "AI.LANG.LISP.CODE.ITER.SERIES")))
	...
	(when (and sym (eq code :internal)
		   (not (boundp sym)) (not (fboundp sym)) (null (symbol-plist sym)))
	  (unintern sym pkg)))
      (use-package "AI.LANG.LISP.CODE.ITER.SERIES" pkg)
      (when shadow (shadowing-import *series-forms* pkg))))
  (when (and remove (member (find-package "AI.LANG.LISP.CODE.ITER.SERIES") (package-use-list pkg)))
    (unuse-package "AI.LANG.LISP.CODE.ITER.SERIES" pkg)
...
(cltl1-eval-when (eval load compile) 
  (proclaim
...
(cltl1-eval-when (eval load compile) (proclaim '(special *state*)))
...
(defS alter (destinations items)
  ...
  (fragL ((destinations) (items T)) ((result))
	 ((gen (or function null)) (result null)) ()
  ...
  (cond ((null expr-list)
        ...
        (T (cl:let ((full-expr-list
			(opt-non-opt `(list ,expr ,@ expr-list)
             ...
             (fragL ((full-expr-list)) ((items T)) ((items T) (lst list)) ()
...
(defS collect-file (name items &optional (printer #'print))
  ...
  (fragL ((name) (items T) (printer)) ((out)) ((out (member T nil)) (lst list)) ()
 ...
 :optimizer
  (funcall-literal-frag
    (cl:let ((file (new-var 'outfile)))
      `((((items T) (printer)) ((out)) ((out (member T nil))) ()
